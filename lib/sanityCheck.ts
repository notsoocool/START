...
export function runSanityCheck(rows: AnalysisRow[]): SanityResult {
    ...
    const refToRowInfo = new Map<string, { slokano: string; sentno: string; anvaya: string }>();
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (["-", "", "."].includes(word)) {
            ...
            for (const { ref } of kaarakaEntries)
                if (ref) {
                    referencedAnvayas.add(ref);
                    if (!refToRowInfo.has(ref)) refToRowInfo.set(ref, { slokano, sentno, anvaya });
                }
            continue;
        }
    ...
    for (const ref of Array.from(referencedAnvayas)) {
        if (!anvayaSet.has(ref)) {
            const rowInfo = refToRowInfo.get(ref);
            if (rowInfo) {
                pushErr(errors, rowInfo.slokano, rowInfo.sentno, rowInfo.anvaya, `Hanging reference: anvaya_no "${ref}" is referenced but does not exist`);
            } else {
